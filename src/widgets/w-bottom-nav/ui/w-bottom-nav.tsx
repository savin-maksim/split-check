...
import './w-bottom-nav.scss'

type TActionConfig = { icon: ReactNode; title: string }

const ACTION_ICONS: Record<string, TActionConfig> = {
  '/': {
    icon: <FilePlus aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
    title: 'Новый чек',
  },
  'people': {
    icon: <UserPlus aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
    title: 'Добавить людей',
  },
  'items': {
    icon: <Plus aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
    title: 'Добавить расход',
  },
  'stats': {
    icon: <Share2 aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
    title: 'Поделиться',
  },
}
...
export const WBottomNav = () => {
  ...
  const navItems = useMemo(() => {
    if (!contextCheckId) {
      return [
        {
          to: '/',
          icon: <Receipt aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
          label: 'Чеки',
        },
      ]
    }
    return [
      ...
      },
      {
        ...
        icon: <Users aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
        label: 'Люди',
      },
      {
        to: buildRoute.items(contextCheckId),
        icon: <Calculator aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
        label: 'Расходы',
      },
      {
        to: buildRoute.stats(contextCheckId),
        icon: <BarChart3 aria-hidden="true" className="w-bottom-nav__icon" size={'var(--bottom-nav-icon-size)'} />,
        label: 'Статистика',
      },
    ]
  }, [contextCheckId])
  ...
  return (
    <nav className="w-bottom-nav" aria-label="Основная навигация">
      {navItems.slice(0, midIndex).map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
          end={item.to === '/'}
          className={({ isActive }) => cn('w-bottom-nav__link', isActive && 'w-bottom-nav__link--active')}
      ...
      {actionConfig && onAction ? (
        <button
          className="w-bottom-nav__action"
          onClick={onAction}
        ...
        <div className="w-bottom-nav__action-placeholder" />
      )}
